...
void main() {
  ...
  group("Request building", () {
    ...
    test("Host created correctly", () {
      ...
      final portConfiguredClient = Agent.fromOptions(ApplicationOptions()..port = 2121);
    ...
    test("Client can expect array of JSON", () async {
      ...
      final server = await HttpServer.bind("localhost", 8888, v6Only: false, shared: false);
      final router = Router();
  ...
  group("Response handling", () {
    ...
    test("Request with accept adds header", () async {
      ...
      server.listen((req) {
        final resReq = Request(req);
        resReq.respond(Response.ok({"ACCEPT": req.headers.value(HttpHeaders.acceptHeader)}));
      });

      final client = Agent.onPort(4000);
      final req = client.request("/foo")..accept = [ContentType.json, ContentType.text];
