...
void main() {
  ...
  group("Project naming", () {
    test("Appropriately named project gets created correctly", () async {
      ...
      expect(Directory.fromUri(cli.agent.workingDirectory.uri.resolve("test_project/")).existsSync(), true);
    });
    ...
    test("Project name with uppercase characters fails immediately", () async {
      ...
      expect(Directory.fromUri(cli.agent.workingDirectory.uri.resolve("test_project/")).existsSync(), false);
    });
    ...
    for (var template in templates) {
      ...
      test("Tests run on template generated from local path", () async {
        ...
        var res = Process.runSync(cmd, ["run", "test", "-j", "1"],
            runInShell: true,
            workingDirectory:
                cli.agent.workingDirectory.uri.resolve("test_project").toFilePath(windows: Platform.isWindows));
