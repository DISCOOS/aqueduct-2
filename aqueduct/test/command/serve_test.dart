...
File get certificateFile =>
    File.fromUri(Directory.current.uri.resolve("../").resolve("ci/").resolve("aqueduct.cert.pem"));

File get keyFile => File.fromUri(Directory.current.uri.resolve("../").resolve("ci/").resolve("aqueduct.key.pem"));
...
void main() {
  ...
  test("Served application starts and responds to route", () async {
    ...
    var thisPubspec = yaml.loadYaml(File.fromUri(Directory.current.uri.resolve("pubspec.yaml")).readAsStringSync());
  ...
  test("Ensure we don't find the base ApplicationChannel class", () async {
    projectUnderTestCli.agent
        .addOrReplaceFile("lib/application_test.dart", "import 'package:aqueduct_2/aqueduct_2.dart';");
...
static Future initializeApplication(ApplicationOptions x) async { throw new Exception("error"); }            
  ...
  test("Start with valid SSL args opens https server", () async {
    certificateFile.copySync(
        projectUnderTestCli.agent.workingDirectory.uri.resolve("server.crt").toFilePath(windows: Platform.isWindows));
    keyFile.copySync(
        projectUnderTestCli.agent.workingDirectory.uri.resolve("server.key").toFilePath(windows: Platform.isWindows));

    task = projectUnderTestCli
        .start("serve", ["--ssl-key-path", "server.key", "--ssl-certificate-path", "server.crt", "-n", "1"]);
    await task.hasStarted;
  ...
  test("Start with invalid SSL values throws exceptions", () async {
    ...
    var badCertFile = File.fromUri(projectUnderTestCli.agent.workingDirectory.uri.resolve("server.crt"));
  ...
  test("Use config-path, absolute path", () async {
    ...
    task = projectUnderTestCli.start("serve", [
      "--config-path",
      projectUnderTestCli.agent.workingDirectory.uri.resolve("foobar.yaml").toFilePath(windows: Platform.isWindows),
      "-n",
      "1"
    ]);
    await task.hasStarted;
