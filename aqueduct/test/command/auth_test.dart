...
void main() {
  ...
  group("Success cases", () {
    ...
    test("Can create confidential client with redirect uri", () async {
      await cli.run("auth", ["add-client", "--id", "a.b.c", "--secret", "abc", "--redirect-uri", "http://foobar.com"]);
    ...
    test("Scope gets collapsed", () async {
      await cli.run("auth", ["add-client", "--allowed-scopes", "xyz:a xyz xyz:a.f xyz.f", "--id", "a.b.c"]);
  ...
  group("Failure cases", () {
    ...
    test("Malformed scope fails", () async {
      final processResult = await cli.run("auth", ["add-client", "--id", "foobar", "--allowed-scopes", "x\"x"]);
      final q = Query<ManagedAuthClient>(context);
      final results = await q.fetch();
      expect(results.length, 0);
