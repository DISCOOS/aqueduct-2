...
void main() {
  ...
  group("Equals matcher", () {
    ...
    test("String value, case insensitive default", () async {
      var q = Query<TestModel>(context)..where((o) => o.email).equalTo("0@A.com", caseSensitive: false);
      var results = await q.fetch();
      expect(results.length, 1);
      expect(results.first.id, 1);

      q = Query<TestModel>(context)..where((o) => o.email).not.equalTo("0@A.com", caseSensitive: false);
      results = await q.fetch();
      expect(results.length, 5);
  ...
  group("Like matcher", () {
    ...
    test("case insensitive default", () async {
      ...
      q = Query<TestModel>(context)..where((o) => o.email).not.like("_@a.COM", caseSensitive: false);
      results = await q.fetch();
      expect(results.length, 0);
    });
  });
  ...
  group("Not equal matcher", () {
    ...
    test("String value, case sensitive default", () async {
      var q = Query<TestModel>(context)..where((o) => o.email).notEqualTo("0@A.com", caseSensitive: false);
      var results = await q.fetch();
      expect(results.length, 5);
      expect(results.any((t) => t.id == 1), false);

      q = Query<TestModel>(context)..where((o) => o.email).not.notEqualTo("0@A.com", caseSensitive: false);
      results = await q.fetch();
      expect(results.length, 1);
      expect(results.any((t) => t.id == 1), true);
    });
