...
void main() {
  ...
  test("Subsequent upgrades do not fail because the verison table is already created", () async {
    final s1 = await store.upgrade(Schema.empty(), [EmptyMigration()..version = 1], temporary: true);
  ...
  test("Trying to upgrade to version that is earlier than latest migration fails", () async {
    final s1 = await store.upgrade(Schema.empty(), [EmptyMigration()..version = 2], temporary: true);
    try {
      await store.upgrade(s1, [EmptyMigration()..version = 1], temporary: true);
      expect(true, false);
    } on MigrationException catch (e) {
  ...
  test("Apply more than one migration to new database", () async {
    await store.upgrade(Schema.empty(), [EmptyMigration()..version = 1, EmptyMigration()..version = 2],
        temporary: true);
    expect(await store.schemaVersion, 2);
  });
  ...
  test("Apply more than one migration to existing database", () async {
    await store.upgrade(Schema.empty(), [EmptyMigration()..version = 1, EmptyMigration()..version = 2],
        temporary: true);
    expect(await store.schemaVersion, 2);
    await store.upgrade(Schema.empty(), [EmptyMigration()..version = 3, EmptyMigration()..version = 4],
        temporary: true);
    expect(await store.schemaVersion, 4);
  });

  test("Can apply an unencoded initial value", () async {
    await store.upgrade(Schema.empty(), [MCase11()..version = 1, MCase12()..version = 2], temporary: true);
    expect(await store.schemaVersion, 2);
...
class MCase11 extends Migration {
  ...
  Future upgrade() async {
    database.createTable(
        SchemaTable("t", [SchemaColumn("id", ManagedPropertyType.integer, isPrimaryKey: true, autoincrement: true)]));
  }
...
class MCase12 extends Migration {
  @override
  Future upgrade() async {
    database.addColumn("t", SchemaColumn("v", ManagedPropertyType.integer), unencodedInitialValue: "10");
  }
