...
void main() {
  ...
  group("Matching", () {
    Schema baseSchema;
    setUp(() {
      var dataModel = ManagedDataModel([LoadedSingleItem, DefaultItem, LoadedItem, Container, Unique]);
    ...
    test("Missing column shows up as error", () {
      ...
      expect(diff.hasDifferences, true);
      expect(diff.errorMessages.length, 1);
      expect(diff.errorMessages.first, contains("Column 'id' in table '_DefaultItem' should exist"));
    });
    ...
    test("Additional column shows up as error", () {
      ...
      expect(diff.hasDifferences, true);
      expect(diff.errorMessages.length, 1);
      expect(diff.errorMessages.first, contains("Column 'foo' in table '_DefaultItem' should NOT exist"));
    });

    test("Same column but with wrong name shows up as error", () {
      var newSchema = Schema.from(baseSchema);
      newSchema.tables.firstWhere((t) => t.name == "_DefaultItem").columns.firstWhere((c) => c.name == "id").name =
          "idd";

      var diff = baseSchema.differenceFrom(newSchema);
      expect(diff.hasDifferences, true);
      expect(diff.errorMessages.length, 2);
      expect(diff.errorMessages, contains(contains("Column 'id' in table '_DefaultItem' should exist")));
      expect(diff.errorMessages, contains(contains("Column 'idd' in table '_DefaultItem' should NOT exist")));
    });

    test("Column differences show up as errors", () {
      var newSchema = Schema.from(baseSchema);
      var column =
          newSchema.tables.firstWhere((t) => t.name == "_DefaultItem").columns.firstWhere((c) => c.name == "id");
    ...
    test("Multiple reasons all show up", () {
      ...
      expect(diff.errorMessages,
          contains('Column \'foobar\' in table \'_DefaultItem\' should NOT exist, but is created by migration files'));
      expect(diff.errorMessages, contains('Table \'foo\' should NOT exist, but is created by migration files.'));
    });
...
class _ExtensiveModel {
  ...
  @Column(databaseType: ManagedPropertyType.bigInteger, nullable: true, defaultValue: "7", unique: true, indexed: true)
  int loadedValue;
}
