...
void main() {
  group("Valid data model", () {
    ...
    setUp(() {
      dataModel = ManagedDataModel([User, Item, Manager, EnumObject, DocumentObject, AnnotatedTable]);
    ...
    test("Relationships have appropriate values set", () {
      ...
      expect(relDesc.inverse == dataModel.entityForType(User).relationships[relDesc.inverseKey], true);
      ...
      expect(relDesc.inverse == dataModel.entityForType(Manager).relationships[relDesc.inverseKey], true);
  ...
  group("Edge cases", () {
    test("ManagedObject with two foreign keys to same object are distinct", () {
      var model =
          ManagedDataModel([DoubleRelationshipForeignKeyModel, DoubleRelationshipHasModel, SomeOtherRelationshipModel]);
      ...
      expect(isManyOf.destinationEntity.tableName, model.entityForType(DoubleRelationshipHasModel).tableName);
      ...
      expect(isOneOf.destinationEntity.tableName, model.entityForType(DoubleRelationshipHasModel).tableName);
    });
