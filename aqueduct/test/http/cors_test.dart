...
void main() {
  ...
  group(
    ...
    test("Valid endpoint returns correctly, mis-matched origin", () async {
      var resp = await http.get(Uri.parse('http://localhost:8000/restrictive'), headers: {"Origin": "not this"});
      expect(resp.statusCode, 200);
      expectThatNoCORSProcessingOccurred(resp);
    });
    ...
    test("Valid endpoint, case match failure", () async {
      var resp =
          await http.get(Uri.parse('http://localhost:8000/restrictive'), headers: {"Origin": "http://Exclusive.com"});
      expect(resp.statusCode, 200);
      expectThatNoCORSProcessingOccurred(resp);
    });
    ...
    test("Invalid resource gets CORS headers to expose 404 to calling client", () async {
      ...
      var resp = await http.get(Uri.parse('http://localhost:8000/foobar'), headers: {"Origin": "http://abc.com"});
      expect(resp.statusCode, 404);
  ...
  group(
    ...
    test(
        ...
        () async {
      var resp =
          await http.get(Uri.parse('http://localhost:8000/defaultpolicy'), headers: {"Origin": "http://foobar.com"});
      expect(resp.statusCode, 200);
    ...
    test("Normal/Simple Requests: If credentials are not supported and origin is valid, only set origin", () async {
      var resp = await http
          .get(Uri.parse('http://localhost:8000/restrictive_nocreds'), headers: {"Origin": "http://exclusive.com"});
      expect(resp.statusCode, 200);
      expect(resp.headers["access-control-allow-origin"], "http://exclusive.com");
      expect(resp.headers["access-control-allow-headers"], isNull);
      expect(resp.headers["access-control-allow-methods"], isNull);
  ...
  group("Generators and policies", () {
    test("Headers don't continue to pile up when using a generator", () async {
      ...
      for (var i = 0; i < 10; i++) {
        lastResponse = await http.get(Uri.parse('http://localhost:8000/add'), headers: {"Origin": "http://www.a.com"});
...
class CORSChannel extends ApplicationChannel with AuthValidator {
  ...
  Controller get entryPoint {
    ...
    router.route("/restrictive_auth").link(() => Authorizer(this)).link(() => RestrictiveOriginController());
    ...
    router.route("/defaultpolicyauth").link(() => Authorizer(this)).link(() => DefaultPolicyController());
    return router;
  }
