...
void main() {
  group("Standard operations", () {
    ...
    test("Can update an object", () async {
      var expectedMap = {"id": 1, "name": "Fred", "createdAt": allObjects.first.createdAt.toIso8601String()};
      ...
      expect(await client.request("/controller/2").get(), hasResponse(200, body: allObjects[1].asMap()));
    });
    ...
    test("Can create an object", () async {
      ...
      expect(resp, hasResponse(200, body: expectedMap));
      expect(await client.request("/controller/${expectedMap["id"]}").get(), hasResponse(200, body: expectedMap));
    });
  ...
  group("Objects that don't exist", () {
    ...
    test("Updating an object returns 404", () async {
      expect(await (client.request("/controller/1")..body = {"name": "Fred"}).put(), hasStatus(404));
    });
  ...
  group("Extended GET requests", () {
    ...
    test("Getting all objects with a unknown sort descriptor order fails", () async {
      expect(await client.request("/controller?sortBy=name,name").get(),
          hasResponse(400, body: {"error": "invalid 'sortBy' format. syntax: 'name,asc' or 'name,desc'."}));
    });

    test("Getting all objects with bad syntax fails", () async {
      var resp = await client.request("/controller?sortBy=name,asc,bar").get();
      expect(resp, hasResponse(400, body: {"error": "invalid 'sortyBy' format. syntax: 'name,asc' or 'name,desc'."}));
    });

    test("Paging after", () async {
      expect(
          await client
              .request("/controller?pageBy=createdAt&pageAfter=${allObjects[5].createdAt.toIso8601String()}")
              .get(),
    ...
    test("Paging before", () async {
      expect(
          await client
              .request("/controller?pageBy=createdAt&pagePrior=${allObjects[5].createdAt.toIso8601String()}")
              .get(),
...
class TestChannel extends ApplicationChannel {
  ...
  Controller get entryPoint {
    ...
    router
        .route("/dynamic/[:id]")
        .link(() => ManagedObjectController.forEntity(context.dataModel.entityForType(TestModel), context));
    return router;
  }
}
