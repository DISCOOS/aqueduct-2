...
void main() {
  ...
  test("A decoder with a subtype always trumps a decoder that matches any subtype", () async {
    ...
    var serverResponse = Response.ok({"key": "value"})..contentType = ContentType("a", "specific", charset: "utf-8");
    server = await bindAndRespondWith(serverResponse);
  ...
  test("Invalid charset sends 415", () async {
    var serverResponse = Response.ok("abcd")..contentType = ContentType("text", "plain", charset: "abcd");
    server = await bindAndRespondWith(serverResponse);
  ...
  test("Encoder that doesn't net out with List<int> safely fails", () async {
    CodecRegistry.defaultInstance.add(ContentType("application", "baddata"), BadDataCodec(), allowCompression: false);
