...
void main() {
  ...
  group("Entity descriptions", () {
    ...
    test("Schema object contains all persistent attributes with correct types", () {
      ...
      expect(
          entity.properties["string"].description, contains("No two objects may have the same value for this field"));
    });
  ...
  group("Controller integration", () {
    ...
    test("Can emit document for ManagedObjectController", () {
      ...
      expect(doc.paths["/model"].operations["get"].responses["200"].content["application/json"].schema.type,
          APIType.array);
      expect(
          doc.paths["/model"].operations["get"].responses["200"].content["application/json"].schema.items.referenceURI
              .path,
          "/components/schemas/Model1");
      expect(doc.paths["/model"].operations["post"].requestBody.content["application/json"].schema.referenceURI.path,
          "/components/schemas/Model1");
