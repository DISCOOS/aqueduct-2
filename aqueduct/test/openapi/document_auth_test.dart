...
void main() {
  group("Operations and security schemes", () {
    ...
    setUpAll(() async {
      doc = await Application.document(TestChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
    });
    ...
    test("Authorizer adds Forbidden and Unauthorized to response components", () {
      ...
      expect(doc.components.responses["InsufficientScope"].content["application/json"].schema.properties["error"].type,
          APIType.string);
      expect(doc.components.responses["InsufficientScope"].content["application/json"].schema.properties["scope"].type,
          APIType.string);
    ...
    test("Bearer Authorizer adds 401 and 403 response to operations", () {
      ...
      expect(
          noVarPath.operations["get"].responses["401"].referenceURI.path, "/components/responses/InsufficientAccess");
  ...
  group("Controller Registration and Scopes", () {
    test("If no controllers added to channel, do not support have flows for oauth2 security type", () async {
      final doc = await Application.document(TestChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
    ...
    test("If only AuthController added to channel, do not support auth code flow", () async {
      ...
      expect(doc.components.securitySchemes["oauth2"].flows["password"].refreshURL, Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["password"].tokenURL, Uri(path: "auth/token"));
    ...
    test(
        ...
        () async {
      final doc =
          await Application.document(ScopedControllerChannel, ApplicationOptions(), {"name": "Test", "version": "1.0"});
      expect(doc.components.securitySchemes["oauth2"].flows["password"].refreshURL, Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["password"].tokenURL, Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["password"].authorizationURL, isNull);

      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].refreshURL, Uri(path: "auth/token"));
      expect(doc.components.securitySchemes["oauth2"].flows["authorizationCode"].tokenURL, Uri(path: "auth/token"));
      expect(
          doc.components.securitySchemes["oauth2"].flows["authorizationCode"].authorizationURL, Uri(path: "auth/code"));
    });
...
class TestChannel extends ApplicationChannel {
  ...
  Controller get entryPoint {
    ...
    router.route("/basic/[:id]").link(() => Authorizer.basic(authServer)).link(() => DocumentedController());
    router.route("/bearer-no-scope").link(() => Authorizer.bearer(authServer)).link(() => DocumentedController());
    router
        .route("/bearer-scope")
  ...
  Map<String, APIOperation> documentOperations(APIDocumentContext components, String route, APIPath path) {
    if (path.containsPathParameters([])) {
      return {
        "get": APIOperation("get/0", {
          "200": APIResponse("get/0-200", content: {"application/json": APIMediaType(schema: APISchemaObject.string())})
        }),
        "post": APIOperation("post/0", {
          "200":
              APIResponse("post/0-200", content: {"application/json": APIMediaType(schema: APISchemaObject.string())})
        })
      };
    }
    ...
    return {
      "get": APIOperation("get/1", {
        "200": APIResponse("get/1-200", content: {"application/json": APIMediaType(schema: APISchemaObject.string())})
      })
    };
  }
...
class ScopedControllerChannel extends ApplicationChannel {
  ...
  Controller get entryPoint {
    ...
    router.route("/r3").link(() => Authorizer.bearer(authServer)).link(() => DocumentedController());
    return router;
  }
}
