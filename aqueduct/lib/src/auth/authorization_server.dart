...
/// implementation like `ManagedAuthDelegate` from `package:aqueduct_2/managed_auth.dart`.
///
...
class AuthServer implements AuthValidator, APIComponentDocumenter {
  ...
  ///
  /// It is preferable to use the implementation of [AuthServerDelegate] from 'package:aqueduct_2/managed_auth.dart'. See
  ...
  /// Used during OpenAPI documentation.
  final APISecuritySchemeOAuth2Flow documentedImplicitFlow = APISecuritySchemeOAuth2Flow.empty()..scopes = {};
  ...
  Future<AuthToken> refresh(String refreshToken, String clientID, String clientSecret,
    ...
    await delegate.updateToken(this, t.accessToken, newToken.accessToken, newToken.issueDate, newToken.expirationDate);
  ...
  /////
  @override
  List<APISecurityRequirement> documentRequirementsForAuthorizer(APIDocumentContext context, Authorizer authorizer,
  ...
  List<AuthScope> _validatedScopes(AuthClient client, ResourceOwner authenticatable, List<AuthScope> requestedScopes) {
    ...
    if (client.supportsScopes) {
      ...
      if (!identical(validScopesForAuthenticatable, AuthScope.any)) {
        validScopes.retainWhere((clientAllowedScope) =>
            validScopesForAuthenticatable.any((userScope) => clientAllowedScope.isSubsetOrEqualTo(userScope)));
  ...
  AuthCode _generateAuthCode(int ownerID, AuthClient client, int expirationInSeconds, {List<AuthScope> scopes}) {
    final now = DateTime.now().toUtc();
    return AuthCode()
