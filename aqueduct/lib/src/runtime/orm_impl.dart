...
class ManagedEntityRuntimeImpl extends ManagedEntityRuntime implements SourceCompiler {
  ...
  List<String> _getValidatorConstructionFromAnnotation(
    ...
    final mirrorOfAnnotationType = buildCtx.context.types
        .firstWhere((t) => MirrorSystem.getName(t.simpleName) == typeOfAnnotationName, orElse: () => null);
    ...
    if (mirrorOfAnnotationType?.isSubtypeOf(reflectType(Validate)) ?? false) {
      ...
      final isSubclassOrInstanceOfValidate = buildCtx.context.getSubclassesOf(Validate).any((subclass) =>
              MirrorSystem.getName(subclass.simpleName) == type.getDisplayString(withNullability: false)) ||
  ...
  String _getValidators(BuildContext context, ManagedPropertyDescription property, {@required List<Uri> importUris}) {
    ...
    final fieldAnnotations = context.getAnnotationsFromField(
        EntityBuilder.getTableDefinitionForType(property.entity.instanceType).reflectedType, property.name);

    final constructorInvocations = fieldAnnotations
        .map((annotation) =>
            _getValidatorConstructionFromAnnotation(context, annotation, property, importUris: importUris))
        .expand((i) => i)
        .toList();
    ...
    final inverseType =
        property is ManagedRelationshipDescription ? "${property.destinationEntity.instanceType}" : "null";
  ...
  List<String> _getConstructorSourcesFromColumnArgList(ArgumentList argList, {@required List<Uri> importUris}) {
  ...
  String _getAttributeInstantiator(BuildContext ctx, ManagedAttributeDescription attribute,
    ...
    final transienceStr = attribute.isTransient
        ...
        : null;
    final validatorStr = attribute.isTransient ? "[]" : "[${_getValidators(ctx, attribute, importUris: importUris)}]";
