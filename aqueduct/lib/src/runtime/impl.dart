...
import 'package:aqueduct_2/src/application/application.dart';
...
import 'package:aqueduct_2/src/application/options.dart';
import 'package:aqueduct_2/src/http/controller.dart';
...
class ChannelRuntimeImpl extends ChannelRuntime implements SourceCompiler {
  ...
  String compile(BuildContext ctx) {
    ...
    final globalInitBody = hasGlobalInitializationMethod ? "await $className.initializeApplication(config);" : "";
...
class ControllerRuntimeImpl extends ControllerRuntime implements SourceCompiler {
  ...
  bool get isMutable {
    ...
    final fieldKeys = type.instanceMembers.keys.where((sym) => !whitelist.contains(MirrorSystem.getName(sym)));
...
class SerializableRuntimeImpl extends SerializableRuntime {
  ...
  static APISchemaObject documentVariable(APIDocumentContext context, VariableMirror mirror) {
    APISchemaObject object = documentType(context, mirror.type)..title = MirrorSystem.getName(mirror.simpleName);
