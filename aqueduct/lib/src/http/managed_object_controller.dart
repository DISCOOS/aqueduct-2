  ...
  Future<Response> getObjects(
    ...
    if (sortBy != null) {
      sortBy.forEach((sort) {
        ...
        if (split.length != 2) {
          throw Response.badRequest(body: {"error": "invalid 'sortyBy' format. syntax: 'name,asc' or 'name,desc'."});
        }
        ...
        if (split.last != "asc" && split.last != "desc") {
          throw Response.badRequest(body: {"error": "invalid 'sortBy' format. syntax: 'name,asc' or 'name,desc'."});
        }
      ...
      case "GET":
        if (operation.pathVariables.isEmpty) {
          return {
            ...
            "400": APIResponse.schema("Invalid request.", APISchemaObject.object({"error": APISchemaObject.string()}))
          };
        }

        return {
          "200": APIResponse.schema("Returns a single object.", context.schema.getObjectWithType(InstanceType)),
          "404": APIResponse("No object found.")
        };
      case "PUT":
        return {
          "200": APIResponse.schema("Returns updated object.", context.schema.getObjectWithType(InstanceType)),
          "404": APIResponse("No object found."),
          "400": APIResponse.schema("Invalid request.", APISchemaObject.object({"error": APISchemaObject.string()})),
          "409":
              APIResponse.schema("Object already exists", APISchemaObject.object({"error": APISchemaObject.string()})),
        };
      case "POST":
        return {
          "200": APIResponse.schema("Returns created object.", context.schema.getObjectWithType(InstanceType)),
          "400": APIResponse.schema("Invalid request.", APISchemaObject.object({"error": APISchemaObject.string()})),
          "409":
              APIResponse.schema("Object already exists", APISchemaObject.object({"error": APISchemaObject.string()}))
        };
